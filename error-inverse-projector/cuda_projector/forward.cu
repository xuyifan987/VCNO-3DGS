...
renderCUDA(
	...
	float* __restrict__ gaussian_errors,
	float* __restrict__ gaussian_effects,
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (inside)
			{
				atomicAdd(&(gaussian_errors[collected_id[j]]), pixel_error * w);
				atomicAdd(&(gaussian_effects[collected_id[j]]), w);
			}
