...
__device__ void siren_backward(
	const float* __restrict__ W1,
	const float* __restrict__ b1,
	const float* __restrict__ W2,
	const float* __restrict__ b2, 
	const int w_base_idx,
	const int g_base_idx,
	const float* __restrict__ input,
	const float* __restrict__ dL_dchannel,
	const float dchannel_dcolor,
	...
	const float omega = (float)SIREN_OMEGA;

	// Forward
	float y[HIDDEN_NEURON];
	liner_transform(
		&(W1[w_base_idx * HIDDEN_NEURON * MLP_INPUT_DIM]),
		&(b1[w_base_idx * HIDDEN_NEURON]),
		input,
		y,
		HIDDEN_NEURON,
		MLP_INPUT_DIM
	...
	float z[HIDDEN_NEURON];
	for (int i = 0; i < HIDDEN_NEURON; i++)
		z[i] = __sinf(omega * y[i]);
	
	float u[MLP_OUTPUT_DIM];
	liner_transform(
		&(W2[w_base_idx * MLP_OUTPUT_DIM * HIDDEN_NEURON]),
		&(b2[w_base_idx * MLP_OUTPUT_DIM]),
		z,
		u,
		MLP_OUTPUT_DIM,
		HIDDEN_NEURON
	...
	// Backward
	float dL_du[MLP_OUTPUT_DIM];
	for (int i = 0; i < MLP_OUTPUT_DIM; i++)
	{
		const float dout_du = sigmoid_backward(u[i]);
		dL_du[i] = dL_dchannel[i] * dchannel_dcolor * dout_du;
		atomicAdd(&(dL_db2[g_base_idx * MLP_OUTPUT_DIM + i]), dL_du[i] * 1.f);
		for (int j = 0; j < HIDDEN_NEURON; j++)
		{
			atomicAdd(&(dL_dW2[g_base_idx * MLP_OUTPUT_DIM * HIDDEN_NEURON + i * HIDDEN_NEURON + j]), dL_du[i] * z[j]);
		}
	}

	float dL_dz[HIDDEN_NEURON];
	mul_vec_mat(
		dL_du,
		&(W2[w_base_idx * MLP_OUTPUT_DIM * HIDDEN_NEURON]),
		dL_dz,
	...
	float dL_dy[HIDDEN_NEURON];
	for (int i = 0; i < HIDDEN_NEURON; i++)
	{
		const float dz_dy = omega * __cosf(omega * y[i]);
		dL_dy[i] = dL_dz[i] * dz_dy;
		atomicAdd(&(dL_db1[g_base_idx * HIDDEN_NEURON + i]), dL_dy[i] * 1.f);
		for (int j = 0; j < MLP_INPUT_DIM; j++)
		{
			atomicAdd(&(dL_dW1[g_base_idx * HIDDEN_NEURON * MLP_INPUT_DIM + i * MLP_INPUT_DIM + j]), dL_dy[i] * input[j]);
		}
	}
...
renderCUDA(
	...
	const float* __restrict__ W2,
	const float* __restrict__ b2,
	const float* __restrict__ appearance_contexts,
	...
	// the effective extent is now depended on the opacity of gaussian.
			...
			input[0] = s.x / cutoff;
			input[1] = s.y / cutoff;
			input[2] = in_dirs[global_id].x;
			input[3] = in_dirs[global_id].y;
			input[4] = in_dirs[global_id].z;
#pragma unroll
			for (int k = 5; k < MLP_INPUT_DIM; k++)
				input[k] = appearance_contexts[global_id * (MLP_INPUT_DIM - 5) + (k - 5)];
			float computed_color[3];
			float dL_dinput[MLP_INPUT_DIM];
			siren_backward(
				W1, b1, W2, b2, 
				global_id, 
				global_id,
			...
			for (int ch = 0; ch < C; ch++)
			{
				...
				const float c = computed_color[ch];
			...
			if (rho3d <= rho2d) {
				...
				const float2 dL_ds = {
					dL_dG * -G * s.x + dL_dinput[0] / cutoff + dL_dz * Tw.x,
					dL_dG * -G * s.y + dL_dinput[1] / cutoff + dL_dz * Tw.y
				};
				...
				atomicAdd(&dL_ddir[global_id * 3 + 0],  dL_dinput[2]);
				atomicAdd(&dL_ddir[global_id * 3 + 1],  dL_dinput[3]);
				atomicAdd(&dL_ddir[global_id * 3 + 2],  dL_dinput[4]);
			} else {
...
__global__ void preprocessCUDA(
	...
	float3 dL_dmean = dnormvdv(float3{ in_dirs[idx].x, in_dirs[idx].y, in_dirs[idx].z }, float3{ dL_ddir[idx * 3 + 0], dL_ddir[idx * 3 + 1], dL_ddir[idx * 3 + 2] });
	dL_dmean3Ds[idx] += glm::vec3(dL_dmean.x, dL_dmean.y, dL_dmean.z);
...
void BACKWARD::render(
	...
	const float* W2,
	const float* b2,
	const float* appearance_contexts,
	const float* transMats,
	const float* depths,
