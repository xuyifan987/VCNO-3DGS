...
__forceinline__ __device__ float sigmoid(float value)
{
	return 1.f / (1.f + exp(-value));
}

__forceinline__ __device__ float sigmoid_backward(float value)
{
	return sigmoid(value) * (1.f - sigmoid(value));
}

// Compute y = Ax + b
__forceinline__ __device__ void liner_transform(
	const float* __restrict__ A, // (JxK)
	const float* __restrict__ b, // (J)
...
// Compute y = (xT)A
__forceinline__ __device__ void mul_vec_mat(
	const float* __restrict__ x, // (J)
	const float* __restrict__ A, // (JxK)
	float* y, // (J)
	const int J, // Row
	const int K // Column
