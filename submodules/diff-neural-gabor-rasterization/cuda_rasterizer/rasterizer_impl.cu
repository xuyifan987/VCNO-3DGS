...
CudaRasterizer::GeometryState CudaRasterizer::GeometryState::fromChunk(char*& chunk, size_t P)
{
	...
	obtain(chunk, geom.in_dir, P, 128);
	return geom;
}
	...
	bool debug)
{
	...
	auto dL_ddir_tensor = torch::zeros({P, 3}, torch::TensorOptions().dtype(torch::kFloat).device(torch::kCUDA)).contiguous();
	float* dL_ddir = dL_ddir_tensor.data_ptr<float>();
	...
	// const float* transMat_ptr = (transMat_precomp != nullptr) ? transMat_precomp : geomState.transMat;
	CHECK_CUDA(BACKWARD::preprocess(P, D, 
		(float3*)means3D,
		radii,
		geomState.clamped,
		(glm::vec2*)scales,
