...
template<int C>
__global__ void preprocessCUDA(int P, int D, 
	...
	const glm::vec3 campos_vec = (glm::vec3)*cam_pos;
	const glm::vec3 pos = ((glm::vec3*)orig_points)[idx];
	glm::vec3 dir = pos - campos_vec;
	dir = dir / glm::length(dir);

	in_dirs[idx] = {dir.x, dir.y, dir.z};
	depths[idx] = p_view.z;
...
// Global and Shared mem version are same.
__device__ void siren_forward(
	...
	liner_transform(
		&(W1[base_idx * HIDDEN_NEURON * MLP_INPUT_DIM]),
		&(b1[base_idx * HIDDEN_NEURON]),
		input,
		y,
		HIDDEN_NEURON,
		MLP_INPUT_DIM
	...
	liner_transform(
		&(W2[base_idx * MLP_OUTPUT_DIM * HIDDEN_NEURON]),
		&(b2[base_idx * MLP_OUTPUT_DIM]),
		z,
		u,
		MLP_OUTPUT_DIM,
		HIDDEN_NEURON
...
renderCUDA(
	...
	const float3* __restrict__ in_dirs
) {
...
#pragma unroll
			...
			// Global mem version
			siren_forward(
				W1, b1, W2, b2, global_id, input, computed_color
			);

			// Eq. (3) from 3D Gaussian splatting paper.
			for (int ch = 0; ch < CHANNELS; ch++)
			{
				C[ch] += computed_color[ch] * w;
			}
			T = test_T;
...
void FORWARD::preprocess(int P, int D, 
	const float* means3D,
	const glm::vec2* scales,
	const float scale_modifier,
	const glm::vec4* rotations,
